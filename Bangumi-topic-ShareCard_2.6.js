...
// @version      2.6
// @description  底部文字替换为 Logo 图片，移除 AI 标签，显示小组名称和回复数，保留布局样式与动画，布局配色方案优化，增加bangumi独家表情随机显示，增加点赞表情显示（复用tag样式）
...
(function() {
    ...
    async function createShareImage() {
        ...
        const base64Emojis = allBase64.slice(4); // 与 likeItems 顺序对应
...
/*         // 自动关闭计时器（15秒）
    ...
    async function generateCardImageForClient() {
        ...
        const base64Logo = allBase64[2];
        const base64Gif = allBase64[3];
        const base64Emojis = allBase64.slice(4);
